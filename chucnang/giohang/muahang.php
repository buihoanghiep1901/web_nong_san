...
<?php  
    if(isset($_POST['submit'])){
        ...
        while($row = mysqli_fetch_array($query)){
            ...
            $strBody .= '<tr>
                <td class="prd-name">'.$row['title'].'</td>
                <td class="prd-price"><font color="#C40000">'.$newprice.' VNĐ</font></td>
                <td class="prd-number">'.$_SESSION['giohang'][$row['id']].'</td>
        ...
        // nạp thư viện
        require("PHPMailer.php"); // nạp thư viện
        require("POP3.php");
        require("SMTP.php");
        ...
        // Phải chỉnh sửa lại
        $mailer->Username = "sytrung2k1@gmail.com"; // GMAIL username
        $mailer->Password = "ctuariiizhfirtks"; // GMAIL password
        $mailer->AddAddress($email, $ten); //email người nhận, $email và $ten là 2 biến đc gán bởi $_POST lấy từ trong form
        $mailer->AddCC("nam016298@gmail.com", "Admin Thế giới nông sản"); // gửi thêm một email cho Admin

        // Chuẩn bị gửi thư nào
        $mailer->FromName = 'Thế giới nông sản'; // tên người gửi
        $mailer->From = 'sytrung2k1@gmail.com'; // mail người gửi
        $mailer->Subject = 'Hóa đơn xác nhận mua hàng từ Thế giới nông sản';
